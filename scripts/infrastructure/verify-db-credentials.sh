...
if [[ -n "${PG_ADMIN_PASS:-}" && -n "${REDIS_PASS:-}" && -n "${DATABASE_URL:-}" ]]; then
    ...
    echo "  - Pod can resolve DNS (pgbouncer.infra.svc.cluster.local)"
    ...
    if [[ -z "${DATABASE_URL:-}" ]]; then
        ...
        echo "  \"{\\\"stringData\\\":{\\\"DATABASE_URL\\\":\\\"postgresql://postgres:\${PG_PASS}@pgbouncer.infra.svc.cluster.local:6432/bengo_erp\\\"}}\""
        echo ""
    fi
fi
